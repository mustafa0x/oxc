...
pub fn visit<'a, 'b: 'a>(
    ...
    if let Some(ref mut then) = block.then {
        ...
        let old_scope = context.scope;
        if let Some(&then_scope) = context.analysis.root.template_scope_map.get(&(block.start + 1))
        {
    ...
    if let Some(ref mut catch) = block.catch {
        ...
        let old_scope = context.scope;
        if let Some(&catch_scope) = context.analysis.root.template_scope_map.get(&(block.start + 2))
        {
