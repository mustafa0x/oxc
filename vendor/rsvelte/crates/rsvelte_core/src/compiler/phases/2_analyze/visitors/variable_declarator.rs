...
fn instance_scope_binding(context: &super::VisitorContext<'_>, name: &str) -> Option<usize> {
    ...
    root.all_scopes.get(root.instance_scope_index)?.declarations.get(name).copied()
}
...
pub fn visit_typed(node: &JsNode, context: &mut VisitorContext) -> Result<(), AnalysisError> {
    ...
    if matches!(context.ast_type, super::AstType::Instance) && context.function_depth == 1 {
        ...
        for name in identifiers {
            if context.analysis.module_scope_declarations.contains_key(&name) {
...
fn extract_paths_typed_recursive(
    ...
    match pattern {
        JsNode::Identifier { name, start, .. } => {
            paths.push(PathEntry { name: name.to_string(), is_rest, start: *start });
        }
...
fn is_expression_defined_typed(node: &JsNode, arena: &crate::ast::arena::ParseArena) -> bool {
    match node {
        ...
        JsNode::LogicalExpression { operator, left, right, .. } if operator == "??" => {
        ...
        JsNode::ConditionalExpression { consequent, alternate, .. } => {
        ...
        JsNode::SequenceExpression { expressions, .. } => {
            ...
            exprs.last().map(|last| is_expression_defined_typed(last, arena)).unwrap_or(false)
        }
...
fn visit_runes_mode_typed(
    ...
    for path in &paths {
        if let Some(binding_idx) =
            context.analysis.root.get_binding(path.name.as_str(), context.scope)
        {
    ...
    if let Some(ref rune_name) = rune {
        ...
        if matches!(rune_name.as_str(), "$state" | "$state.raw" | "$derived" | "$derived.by")
                    ...
                    if let Some(bi) = bi {
                        ...
                        b.initial = extract_literal_string_typed(arg)
                            .or_else(|| {
                                ...
                                if let JsNode::UnaryExpression { operator, argument, .. } = arg
                                    && operator == "void"
                                    && matches!(
    ...
    for path in paths {
        ...
        let binding_idx = if rune == "$props" {
            context
                ...
                .or_else(|| context.analysis.root.get_binding(path.name.as_str(), context.scope))
                .or_else(|| {
                    context.analysis.root.scope.declarations.get(path.name.as_str()).copied()
                })
        } else {
            ...
            context.analysis.root.get_binding(path.name.as_str(), context.scope).or_else(|| {
                context.analysis.root.scope.declarations.get(path.name.as_str()).copied()
            })
        };
...
fn process_props_declaration_typed(
    ...
    let custom_elem_has_no_props =
        context.analysis.custom_element.as_ref().is_some_and(|ce| ce.props.is_none());
...
fn process_props_object_pattern_typed(
    ...
    for property in arena.get_js_children(*properties) {
        ...
        let JsNode::Property { start, end, computed, key, value, .. } = property else {
            continue;
        };
        ...
        let JsNode::Identifier { name: value_name, .. } = binding_name_node else {
            return Err(errors::props_invalid_pattern().at(*start, *end));
        };
        ...
        let prop_binding_idx = context
            ...
            .or_else(|| context.analysis.root.scope.declarations.get(value_name.as_str()).copied());
        if let Some(binding_idx) = prop_binding_idx {
            ...
            if let Some(init) = initial_node {
                if let JsNode::CallExpression { callee, arguments, .. } = init {
...
fn visit_non_runes_mode_typed(
    ...
    if let Some(init) = init_node
        && let JsNode::CallExpression { callee, start, end, .. } = init
    {
        let callee_node = arena.get_js_node(*callee);
        if let JsNode::Identifier { name, .. } = callee_node
