...
fn run(source: &str) -> String {
    ...
    let options =
        CompileOptions { filename: Some("App.svelte".to_string()), ..CompileOptions::default() };
...
fn ast_matches_oracle_async_if_block() {
    ...
    for (name, src) in
        &[("async-if-hoisting", hoisting), ("async-if-alternate-hoisting", alternate_hoisting)]
    {
        let (ours, oracle) = run_async_both(src);
        let matched = norm_blocks(&ours) == norm_blocks(&oracle);
        assert!(
...
fn ast_matches_oracle_async_each_block() {
    ...
    for (name, src) in
        &[("async-each-hoisting", hoisting), ("async-each-fallback-hoisting", fallback_hoisting)]
    {
        let (ours, oracle) = run_async_both(src);
        let matched = norm_blocks(&ours) == norm_blocks(&oracle);
        assert!(
...
fn ast_matches_oracle_let_directives() {
    ...
    assert!(mismatches.is_empty(), "let-directive output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_class_state_field_ctor() {
    ...
    let ours = run(src);
    let oracle = oracle_dump(src);
    assert_eq!(norm(&ours), norm(&oracle), "\n--- OURS ---\n{ours}\n--- ORACLE ---\n{oracle}\n");
}
...
fn ast_matches_oracle_simple_samples() {
    ...
    assert!(mismatches.is_empty(), "AST output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_constant_fold() {
    ...
    assert!(mismatches.is_empty(), "constant-fold output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_attribute_fold() {
    ...
    assert!(mismatches.is_empty(), "attribute-fold output differs from oracle for: {mismatches:?}");
}
...
fn snippet_block_hoisted() {
    ...
    assert!(out.contains("function foo($$renderer)"), "missing hoisted snippet function:\n{out}");
...
fn ast_matches_oracle_runtime_burndown_cluster() {
    ...
    for (dir, is_async) in fixtures {
        ...
        let (ours, oracle) =
            if *is_async { run_async_both(&src) } else { (run(&src), oracle_dump(&src)) };
...
fn ast_matches_oracle_structural_samples() {
    ...
    assert!(mismatches.is_empty(), "AST structural output differs from oracle for: {mismatches:?}");
}
...
fn component_call_portion_matches_shape() {
    ...
    let cases = [
        ("<script>import Foo from './Foo.svelte'</script><Foo />", "Foo($$renderer, {});", true),
        (
    ...
    let call_line = |dump: &str| -> Option<String> {
        dump.lines().map(str::trim).find(|l| l.starts_with("Foo($$renderer")).map(str::to_string)
    };
...
fn ast_matches_oracle_component_props() {
    ...
    assert!(failures.is_empty(), "component props-object differs from oracle for: {failures:?}");
}
...
fn ast_matches_oracle_component_children() {
    ...
    let cases: &[(&str, &str)] = &[
        ...
        ("snippet-and-body", "<Foo>{#snippet header()}h{/snippet}body</Foo>"),
    ];
    ...
    assert!(failures.is_empty(), "component children/slots differ from oracle for: {failures:?}");
}
...
fn ast_matches_oracle_dynamic_component() {
    ...
    assert!(failures.is_empty(), "dynamic-component guard differs from oracle for: {failures:?}");
}
...
fn canon_js(code: &str) -> String {
    ...
    oxc_codegen::Codegen::new().with_options(options).build(&parsed.program).code
}
...
fn ast_matches_oracle_inspect_dev_cluster() {
    ...
    let samples: &[(&str, &str, &[&str])] = &[
        ...
        (
            ...
            &["console.log('$inspect(', map, ')');", "console.log('$inspect(', set, ')');"],
        ),
        (
            "inspect-derived-2",
            "<script>\n\tlet state = $derived(1);\n\t$inspect(state);\n</script>\n<p>{state}</p>",
    ...
    assert!(mismatches.is_empty(), "dev-mode inspect SSR lowering differs for: {mismatches:?}");
}
...
fn ast_matches_oracle_dev_instrumentation() {
    ...
    let inline: &[(&str, &str)] = &[
        ...
        ("component-child-snippet", "<A>{#snippet child({ value })}{value}{/snippet}</A>"),
        (
...
fn ast_matches_oracle_props_id() {
    let (ours, oracle) =
        run_both_opts("<script>const id = $props.id();</script><p>{id}</p>", |_| {});
    assert!(
        ours.contains("const id = $.props_id($$renderer);"),
        "props_id re-emission missing:\n{ours}"
    );
    assert!(!ours.contains("$props.id"), "original $props.id() must be dropped:\n{ours}");
    assert_eq!(
        norm(&ours),
        norm(&oracle),
...
fn ast_matches_oracle_component_bindings_settle_loop() {
    ...
    assert!(ours.contains("let $$settled = true;"), "$$settled declaration missing:\n{ours}");
    assert!(
...
fn ast_matches_oracle_css_inject() {
    let (ours, oracle) =
        run_both_opts("<p class=\"x\">hi</p><style>.x { color: red; }</style>", |o| {
            o.css = crate::compiler::CssMode::Injected
        });
    assert!(ours.contains("const $$css = {"), "module-scope $$css const missing:\n{ours}");
    assert!(ours.contains("$$renderer.global.css.add($$css)"), "css.add call missing:\n{ours}");
...
fn ast_matches_oracle_component_api_v4() {
    ...
    assert!(ours.contains("export default App;"), "v4 export default identifier missing:\n{ours}");
    assert_eq!(
        norm(&ours),
        norm(&oracle),
...
fn ast_matches_oracle_whitespace_samples() {
    ...
    assert!(mismatches.is_empty(), "AST whitespace output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_element_binds() {
    ...
    assert!(mismatches.is_empty(), "element-bind codegen differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_script_samples() {
    ...
    let cases: &[(&str, &str)] = &[
        // $state(0) -> let count = 0;
        ("<script>let count = $state(0);</script><p>{count}</p>", "let count = 0;"),
        ...
        // $props() -> let { a } = $$props;
        ("<script>let { a } = $props();</script><p>{a}</p>", "let { a } = $$props;"),
        // $derived(literal) -> let d = $.derived(() => 2 + 3);
        ("<script>let d = $derived(2 + 3);</script><p>{d}</p>", "let d = $.derived(() => 2 + 3);"),
        // import hoisted + $state(x) -> let c = x;
        ("<script>import { x } from './x.js'; let c = $state(x);</script><p>{c}</p>", "let c = x;"),
        // $state.raw -> bare arg
        ("<script>let r = $state.raw(7);</script><p>x</p>", "let r = 7;"),
        ...
        ("<script>class C { count = $state(0); }</script><p>x</p>", "count = 0;"),
        // (a) class-field $state.raw -> `r = 7`
        ("<script>class C { r = $state.raw(7); }</script><p>x</p>", "r = 7;"),
    ...
    assert!(failures.is_empty(), "instance-script lowering differs from oracle for: {failures:?}");
}
...
fn ast_matches_oracle_props_slots_deconflict() {
    ...
    assert!(failures.is_empty(), "$props slots-deconflict differs from oracle for: {failures:?}");
}
...
fn ast_matches_oracle_props_bindable_default() {
    let cases: &[(&str, &str)] = &[
        ...
        ("<script>let { v = $bindable(5) } = $props();</script>{v}", "let { v = 5 } = $$props;"),
        (
...
fn ast_matches_oracle_class_field_runes_everywhere() {
    let cases: &[(&str, &str)] = &[
        ...
        ("<script>const C = class { foo = $state(0); };</script>", "foo = 0;"),
    ];
    let mut failures = Vec::new();
    for (src, must_have) in cases {
        let ours = run(src);
    ...
    assert!(failures.is_empty(), "class-field rune lowering differs from oracle for: {failures:?}");
}
...
fn nested_class_field_rune_lowered() {
    ...
    assert!(norm(&ours).contains("y = 2;"), "nested class field not lowered:\n{ours}");
    assert!(!norm(&ours).contains("$state"), "residual $state in nested class output:\n{ours}");
}
...
fn declarator_init_reparse_roundtrip() {
    ...
    let cases: &[(&str, &str)] = &[
        ...
        ("<script>let r = $state(0); let foo = 'bar';</script><p>x</p>", "let foo = 'bar';"),
        // plain non-rune object literal init (the block-vs-object gap)
        ("<script>let r = $state(0); let o = { a: 1 };</script><p>x</p>", "let o = { a: 1 };"),
        // plain non-rune array init
        ("<script>let r = $state(0); let arr = [1, 2];</script><p>x</p>", "let arr = [1, 2];"),
        ...
        // non-rune call init
        ("<script>let r = $state(0); let n = foo();</script><p>x</p>", "let n = foo();"),
        // $state with an object literal init
        ("<script>let n = $state({ x: 1 });</script><p>x</p>", "let n = { x: 1 };"),
        ...
        ("<script>let s = $state([1, 2]);</script><p>x</p>", "let s = [1, 2];"),
    ];
    let mut failures = Vec::new();
    for (src, must_have) in cases {
        let ours = run(src);
    ...
    assert!(failures.is_empty(), "declarator init reparse differs from oracle for: {failures:?}");
}
...
fn props_id_dropped() {
    ...
    assert!(!out.contains("$props.id"), "$props.id declarator should be dropped:\n{out}");
    assert!(
...
fn typescript_script_oracle_parity() {
    ...
    assert!(mismatches.is_empty(), "TS AST output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_read_wrapping() {
    ...
    let cases: &[(&str, &[&str], &[&str])] = &[
        ...
        ("<p>{$$props.x}</p>", &["$.escape($$sanitized_props.x)"], &["$.escape($$props.x)"]),
        ...
        ("<script>let d = $derived(fn);</script><p>{d(1)}</p>", &["$.escape(d()(1))"], &[]),
    ...
    assert!(failures.is_empty(), "read-wrapping shape wrong for: {failures:?}");
}
...
fn ast_matches_oracle_bind_props_trailer() {
    let cases: &[(&str, &str)] = &[
        ...
        ("<script>export let name;</script><p>{name}</p>", "$.bind_props($$props, { name });"),
    ];
    let mut failures = Vec::new();
    for (src, must_have) in cases {
        let ours = run(src);
    ...
    assert!(failures.is_empty(), "bind_props trailer differs from oracle for: {failures:?}");
}
...
fn ast_matches_oracle_store_subs_entry() {
    ...
    let orn = norm(&oracle);
    eprintln!("--- AST ---\n{ours}\n--- ORACLE ---\n{oracle}\n");
    for must in ["var $$store_subs;", "if ($$store_subs) $.unsubscribe_stores($$store_subs);"] {
        let want = norm(must);
        assert!(on.contains(&want), "AST output missing `{must}`:\n{ours}");
        assert!(orn.contains(&want), "ORACLE missing `{must}` (sanity):\n{oracle}");
    }
...
fn ast_matches_oracle_legacy_script_samples() {
    ...
    let cases: &[(&str, &str)] = &[
        ...
        ("<script>export let name;</script><p>{name}</p>", "let name = $$props['name'];"),
    ...
    assert!(failures.is_empty(), "legacy-script lowering differs from oracle for: {failures:?}");
}
...
fn compile_both(source: &str) -> Outcome {
    ...
    Outcome::Compared { matched_text, matched_struct, used_fallback, new_canon, oracle_canon }
}
...
fn trunc(s: &str) -> String {
    if s.len() > 70 {
        format!("{}…", &s[..s.char_indices().nth(70).map(|(i, _)| i).unwrap_or(s.len())])
    } else {
        s.to_string()
    }
}
...
fn corpus_new_vs_oracle_inner() {
        ...
        match compile_both(&source) {
            Outcome::Compared {
                ...
                if matched_struct {
                    ...
                    let key_feats: Vec<&'static str> =
                        if feats.is_empty() { vec!["<plain-markup>"] } else { feats };
                    for f in &key_feats {
    ...
    eprintln!("  skipped (parse/analyze fail) .... {skipped} ({:.1}%)", pct(skipped, total));
    eprintln!("  ERROR new=None (unimplemented) .. {new_none} ({:.1}%)", pct(new_none, total));
    eprintln!("  ERROR panic ..................... {panicked} ({:.1}%)", pct(panicked, total));
    eprintln!("  COMPARED (both produced output).. {compared} ({:.1}%)", pct(compared, total));
    ...
    for (sig, (count, examples)) in lines.iter().take(20) {
        ...
        eprintln!("         e.g. {}", examples.first().cloned().unwrap_or_default());
    }
    ...
    for (label, _) in feats.iter().take(5) {
        if let Some((fname, new_canon, oracle_canon)) = feature_repr.get(*label) {
            eprintln!("\n##### cluster `{label}` — {fname}");
            eprintln!("   first-diff: {}", first_diff_line(new_canon, oracle_canon));
...
fn reactive_statement_ssr_shapes() {
    let cases: &[(&str, &str)] = &[
        ...
        ("conditional", "<script>let x = 0;\nlet y = 0;\n$: if (x) y = 1;</script>{y}"),
        ("destructure-assign", "<script>let obj = {a:1};\n$: ({ a } = obj);</script>{a}"),
        ("expr-stmt", "<script>let count = 0;\n$: console.log(count);</script>"),
        ("interdep-source-order", "<script>let a = 1;\n$: b = a * 2;\n$: c = b + 1;</script>{c}"),
        (
            "store-read",
            "<script>import {writable} from 'svelte/store';\nlet x = writable(0);\n$: doubled = $x * 2;</script>{doubled}",
        ),
        ("prop-read", "<script>export let count;\n$: doubled = count * 2;</script>{doubled}"),
        (
            "store-in-block",
            "<script>import {writable} from 'svelte/store';\nlet x = writable(0);\nfunction log(v){}\n$: { log($x); }</script>",
        ),
    ...
    for (name, src) in cases {
        let ours = run(src);
        let oracle = oracle_dump(src);
        let n: Vec<&str> = ours.lines().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
        let o: Vec<&str> = oracle.lines().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
        assert_eq!(
            n, o,
...
fn ast_matches_oracle_three_ssr_fixtures() {
    ...
    assert!(mismatches.is_empty(), "SSR output differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_derived_readwrap_cluster() {
    ...
    assert!(mismatches.is_empty(), "derived read-wrap SSR differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_effect_rune_cluster() {
    ...
    assert!(mismatches.is_empty(), "effect-rune SSR differs from oracle for: {mismatches:?}");
}
...
fn ast_matches_oracle_async_blocker_wrap() {
    let fixtures = ["async-each-derived", "async-dynamic-component", "async-render-hydration"];
    let mut mismatches = Vec::new();
    for dir in fixtures {
        let path = format!(
            "{}/../../submodules/svelte/packages/svelte/tests/runtime-runes/samples/{}/main.svelte",
...
fn ast_matches_oracle_async_attribute_hoisting() {
    let samples = [
        ...
        ("element-style-directive", "<div style:color={await p}></div>"),
        ("element-spread", "<div {...await p}></div>"),
        ("element-object-spread-await", "<div {...{ 'data-x': String(await p) }}></div>"),
        (
            "element-object-spread-nested-await",
            "<div {...{ 'data-x': (async () => await p)() }}></div>",
        ),
        ("custom-element-class-directive", "<my-element class:on={await p}></my-element>"),
        ("custom-element-style-directive", "<my-element style:color={await p}></my-element>"),
        (
        ...
        ("dynamic-element-spread", "<svelte:element this={'div'} {...await p}></svelte:element>"),
        (
        ...
        ("component-object-spread-await", "<Component {...{ x: (await p) + state }} />"),
    ];
