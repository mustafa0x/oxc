...
function getExternalCommentsForRoundTrip(): ExternalCommentReport[] | null {
...
function collectExternalCommentNodes(
  node: unknown,
  ...
  for (let i = 0, len = keys.length; i < len; i++) {
    const key = keys[i]!;
    if (key === "comments" || key === "tokens" || key === "parent") continue;

    const child = node[key];
    if (Array.isArray(child)) {
      for (let j = 0, childLen = child.length; j < childLen; j++) {
        collectExternalCommentNodes(child[j], visitorKeys, seen, collected);
      }
    } else {
      collectExternalCommentNodes(child, visitorKeys, seen, collected);
    }
  }
}

function collectExternalCommentInputs(
  program: Program,
  visitorKeys: VisitorKeysRecord | null,
): unknown[] | null {
  const collected: unknown[] = [];
  collectExternalCommentNodes(program, visitorKeys, new WeakSet(), collected);
...
function serializeLintSuccess(
  ...
  const externalComments = sourceText === null ? null : getExternalCommentsForRoundTrip();

  // Avoid JSON serialization in the common case where there is nothing to report or round-trip.
  if (diagnostics.length === 0 && externalComments === null) {
    return null;
  }
  ...
  return JSON.stringify({
    Success:
      sourceText === null
        ? diagnostics
        : { diagnostics, comments: externalComments ?? [] },
  });
}
...
function setupExternalParserSource(
  ...
  const explicitCommentsInput = pickExternalMetadataArray(program.comments, parserResultCommentsInput);
  const commentsInput =
    explicitCommentsInput !== null && explicitCommentsInput.length > 0
      ? explicitCommentsInput
      : collectExternalCommentInputs(program, visitorKeys);
