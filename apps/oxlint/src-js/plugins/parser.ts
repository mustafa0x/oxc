...
import type { BufferWithArrays } from "./types.ts";
...
let parserBlockBuffer: Uint8Array | null = null;
...
export function parseProgram(
  ...
  const program = deserializeProgramOnly(buffer, sourceText, sourceStartPos, sourceByteLen);
...
function parseIntoBuffer(
  ...
  const blockBuffer = parserBlockBuffer!;
  ...
  parseRawSync(filename, blockBuffer, sourceStartPos, sourceByteLen, normalizeParserOptions(options));
...
function initParserBuffer(): void {
  ...
  parserBuffer.int32 = new Int32Array(arrayBuffer, offset, BUFFER_SIZE / 4);
  ...
  parserBlockBuffer = new Uint8Array(arrayBuffer, offset, BLOCK_SIZE);
}
