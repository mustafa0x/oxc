...
export function setupExternalTokensForFile(
  ...
  for (const tokenInput of tokensInput ?? []) {
    ...
    if (type === "HTMLText" && value.trim().length === 0) continue;
  ...
  if (tokensWithRegexIndexes.length < tokensLen) {
    tokensWithRegexIndexes = new Int32Array(
      Math.max(
        tokensLen,
  ...
  tokensInt32 = new Int32Array(tokensBuffer);
  ...
  for (let i = 0; i < tokensLen; i++) {
    ...
    const pos32 = pos >> 2;
    tokensInt32[pos32] = normalizedToken.start;
    tokensInt32[pos32 + 1] = normalizedToken.end;
