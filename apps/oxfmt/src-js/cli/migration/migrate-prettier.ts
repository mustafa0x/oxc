...
export async function runMigratePrettier() {
  ...
  const prettierConfigPath =
    resolvedPrettierConfigPath ?? (await findFallbackPrettierConfigFile(cwd));
  ...
  } else if (await rawConfigHasOverrides(prettierConfigPath)) {
...
function migratePrettierOptions(
  ...
  for (const [key, value] of Object.entries(prettierConfig)) {
    ...
    // Per-file options must not leak into a shared config.
    ...
    if (key === "requirePragma" || key === "insertPragma") {
      warnMigration(scope, `"${key}" is not supported, skipping...`);
      continue;
    }
  ...
  if (scope.applyDefaults) {
    ...
    oxfmtrc.embeddedLanguageFormatting !== undefined &&
    oxfmtrc.embeddedLanguageFormatting !== "off"
  ) {
...
function migratePlugins(
  ...
  for (const plugin of plugins ?? []) {
    ...
    if (recognizedPluginSpec) {
      if (recognizedPluginSpec.reason === "svelte") {
        warnMigration(
          scope,
          "plugins: prettier-plugin-svelte is replaced by native rsvelte formatting, skipping...",
        );
        continue;
      }
      usesSortPackageJsonPlugin = processMigratablePluginSpec(
      ...
      pushUnique(
        currentMigrationReport.inferredPluginSpecs,
        normalizePreservedPluginSpec(recognizedPluginSpec.spec, projectDir),
      );
      warnMigration(
        scope,
...
function processMigratablePluginSpec(
  ...
  if (pluginSpec.includes("prettier-plugin-svelte")) {
    warnMigration(
      scope,
      "plugins: prettier-plugin-svelte is replaced by native rsvelte formatting, skipping...",
    );
...
function looksLikePrettierPluginRecord(plugin: Record<string, unknown>): boolean {
  return (
    ...
    isRecord(plugin.parsers) ||
    isRecord(plugin.printers) ||
    isRecord(plugin.options)
  );
}
...
function looksLikeSvelteFormatterPlugin(plugin: Record<string, unknown>): boolean {
  ...
  const hasSvelteLanguage = languages.some((language) => {
    ...
    const vscodeLanguageIds = isStringArray(language.vscodeLanguageIds)
...
async function findFallbackPrettierConfigFile(cwd: string): Promise<string | null> {
  for (const configBasename of MANUAL_PRETTIER_CONFIG_DISCOVERY_BASENAMES) {
    ...
    if (configBasename === "package.json") {
      const packageJsonPrettierConfig = await resolveRawJSONPrettierConfig(
        candidatePath,
        configBasename,
      );
...
async function unwrapImportedPrettierConfig(
...
function printMigrationSummary(report: MigrationReport): void {
  ...
  if (report.preservedPluginSpecs.length > 0) {
    console.error(
      `  - preserved formatter plugins: ${formatSummaryList(report.preservedPluginSpecs)}`,
    );
  }
  if (report.inferredPluginSpecs.length > 0) {
    console.error(
      `  - plugin objects converted to string specs: ${formatSummaryList(report.inferredPluginSpecs)}`,
    );
  }
  ...
  if (report.skippedCustomPluginObjects > 0) {
    console.error(
      `  - unsupported custom plugin objects skipped: ${report.skippedCustomPluginObjects}`,
    );
  }
}
